...
const ChartTooltip: React.FC<{
  ...
  return (
    ...
    >
      <p style={{ color: colors.text, marginBottom: 8, fontWeight: 600, fontSize: 11 }}>
        {dateStr}
      </p>
      {payload.map((p, i) => {
        ...
        return (
          ...
          >
            ...
            {delta !== null && (
              <span
                style={{ fontSize: 10, fontWeight: 600, color: delta >= 0 ? '#10b981' : '#f43f5e' }}
              >
                {delta >= 0 ? '+' : ''}
                {delta.toFixed(1)}%
              </span>
...
const AnalysisPage: React.FC = () => {
  ...
  const avgRetirement = useMemo(
    () =>
      n === 0
        ? 0
        : chartData.reduce((s: number, d: ChartDataPoint) => s + (d.Retirement || 0), 0) / n,
    [chartData, n]
  );
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Area
                      ...
                      fill="url(#gradDebt)"
                      dot={false}
                      activeDot={{ r: 4, stroke: '#fff', strokeWidth: 2, fill: '#f43f5e' }}
                    />
                  </AreaChart>
                </ResponsiveContainer>
              </LazyChart>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData}>
                    ...
                    <Area
                      ...
                      dot={retirementLastDot}
                      activeDot={{ r: 5, stroke: '#fff', strokeWidth: 2, fill: '#7c3aed' }}
                    />
                  </AreaChart>
                </ResponsiveContainer>
              </LazyChart>
