  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
          <StatCard label={t('totalIncome')} value={totals.income} color="green" icon={<TrendingUp size={18} />} trend={trends?.income} />
          <StatCard label={t('totalDebt')} value={totals.expenses} color="red" icon={<TrendingDown size={18} />} trend={trends?.debt} trendInverted />
          <StatCard label={t('netBalance')} value={totals.balance} color="blue" icon={<Wallet size={18} />} trend={trends?.balance} />
          <StatCard label={t('retirementCapital')} value={totals.informative} color="yellow" icon={<PiggyBank size={18} />} trend={trends?.retirement} />
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 items-start">
        {loading && categories.length === 0
          ? [...Array(3)].map((_, i) => (
            <div key={i} className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm h-[450px] flex flex-col">
              ...
              <div className="flex-1 overflow-hidden">
                {[...Array(4)].map((_, j) => (
                  <div key={j} className="flex items-center px-5 py-3.5 border-b border-slate-50 dark:border-slate-700/50 gap-3">
